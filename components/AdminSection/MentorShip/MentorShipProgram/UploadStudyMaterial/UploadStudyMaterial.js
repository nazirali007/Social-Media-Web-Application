import { Card, CardMedia, Grid, IconButton, Stack, TextField, } from '@mui/material'
...
const UploadStudyMaterial = () => {
    ...
    return (
        <MDBox width={"100%"} display={"flex"} alignItems={"center"} justifyContent={"center"}>
            <Card sx={{ padding: "2rem", width: "100%" }} component={'form'} >
                <Grid container spacing={3}>
                    ...
                    <Grid item xs={12} sm={5} md={4} spacing={2} sx={{ height: state?.files.length == 0 ? 0 : 200, overflowY: "auto", width: "100%", overflowX: "hidden" }} >
                        {state?.files.map((image, index) => (
                            <Grid key={index} >
                                <Grid sx={{
                                    position: 'relative', width: '70%', height: "120px"
                                }}>
                                    <IconButton sx={{ position: 'absolute', top: 1, left: 165, cursor: "pointer" }}
                ...
                <Grid container spacing={2} my={2}>
                    ...
                    <Grid item xs={12} sm={12} md={7.8} >
                        ...
                        />
                        <MDTypography sx={{ textAlign: "end" }} variant={"h6"}>{title.length}/100</MDTypography>
                    </Grid>
                </Grid>
                                ...
                                {state?.pdfFiles.map((file, index) => {
                                    return (
                                        <MDBox key={index} sx={{ marginTop: 2 }}>
                                            ...
                                            <MDTypography variant='h6' fontWeight={"regular"} >Name: {file.name}</MDTypography>
                                            <MDTypography variant='h6' fontWeight={"regular"} >Type: {file.type}</MDTypography>
                                            <MDTypography variant='h6' fontWeight={"regular"} >Size: {(file.size / 1024).toFixed(2)} KB</MDTypography>
                                        </MDBox>
                                    )
                                })}
                ...
                <MDBox display={"flex"} alignItems={"center"} justifyContent={"center"} width={"100%"} mt={4} >
